...
// Maps every color found in any asset SVG to the projects and assets using it, so the
// project colors list can offer cross-project navigation from a single hover target.
function collectColorUsageAcrossAssets() {
  return Promise.all(assetData.map((asset) => (
    loadAssetColors(asset.source)
      .then((colors) => ({ asset, colors: Array.isArray(colors) ? colors : [] }))
      .catch(() => ({ asset, colors: [] }))
  ))).then((entries) => {
    const usage = new Map();
    for (const { asset, colors } of entries) {
      ...
      for (const color of colors) {
        ...
        if (!usage.has(key)) usage.set(key, { color: key, projects: new Map() });
...
function renderProjectIconColors(projectName) {
  ...
  collectColorUsageAcrossAssets().then(({ usage, entries }) => {
    ...
    for (const color of Array.from(projectColors).sort()) {
      ...
      heading.textContent = `${color} · used in ${projectNames.length} project${projectNames.length === 1 ? "" : "s"}`;
